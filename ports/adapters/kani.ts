/**
 * Kani model-checker adapter + shared spawn runner.
 *
 * This file hosts:
 *   1. The shared {@link AdapterOptions}, {@link SpawnRunner}, {@link SpawnResult},
 *      {@link defaultSpawnRunner}, and {@link formatSpawnFailure} used by every
 *      adapter (kept here to keep this repair within the 5-file budget).
 *   2. The {@link KaniVerifier} class implementing {@link ContractVerifier}
 *      with fail-closed semantics.
 *
 * Fail-closed contract: an adapter MUST NOT return `ok: true` or a synthetic
 * proof unless a configured real backend was invoked and returned valid
 * evidence. Missing tool, timeout, nonzero exit, negative result, and
 * malformed output are surfaced as `ok: false` with a `counterexample`.
 */
...
export async function runAdapter(
  ...
  const result = await runner.run(options.command, {
    stdin: payload,
...
export function toVerdict(outcome: {
  ok: boolean;
  reason: string;
  proof?: string;
  durationMs: number;
